...
SIGNATURE_WASM="../output/signature.wasm"
deploy() {
    mxpy --verbose contract deploy --bytecode=${SIGNATURE_WASM} --recall-nonce --pem=${ALICE} --gas-limit=50000000 --send --outfile="deploy-testnet.interaction.json" --proxy=${PROXY} --chain=T || return
